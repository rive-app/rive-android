...
class CommandQueue internal constructor(
    internal val renderContext: RenderContext,
    private val bridge: CommandQueueBridge = createPlatformBridge(),
    ...
    internal constructor(
        ...
        backendGate: (RenderBackend) -> RenderBackend = ::effectiveRenderBackend,
    ...
    companion object {
        ...
        private val nextShutdownThreadID = AtomicLong(0)
        ...
        private fun createNativeCommandQueueResources(
            ...
            bridge: CommandQueueBridge,
            backendGate: (RenderBackend) -> RenderBackend = ::effectiveRenderBackend,
            renderContextFactory: (RenderBackend) -> RenderContext = ::createPlatformRenderContext,
        ): NativeCommandQueueResources {
            if (backendGate(renderBackend) == RenderBackend.OpenGL) {
     ...
     */
    private val shutdownComplete = ShutdownSignal()
    ...
    private fun dispose(cppPointer: Long) {
        ...
        val shutdownThreadID = nextShutdownThreadID.fetchAndIncrement()
        val shutdownThreadName = "RiveWorkerShutdown-$shutdownThreadID"

        runOnDaemonThread("$shutdownThreadName-Watchdog") {
            if (!shutdownComplete.await(SHUTDOWN_WATCHDOG_TIMEOUT_SECONDS * 1000)) {
        ...
        runOnDaemonThread(shutdownThreadName) {
            ...
            try {
                ...
                return@runOnDaemonThread
            }

            try {
                listeners.close()
                // May throw RiveShutdownException - still need to signal completion.
                renderContext.close()
            } finally {
                shutdownComplete.signal()
            }
    ...
    internal fun awaitShutdown(timeoutMillis: Long): Boolean =
        shutdownComplete.await(timeoutMillis)
    ...
    internal fun nextDrawKey() = DrawKey(bridge.cppCreateDrawKey(cppPointer.pointer))

    /** Deletes a native surface resource. See [CommandQueueBridge.cppDeleteSurface]. */
    internal fun deleteSurfaceNative(surfacePointer: Long) = bridge.cppDeleteSurface(surfacePointer)

    /** Resizes a native surface render target. See [CommandQueueBridge.cppResizeSurface]. */
    internal fun resizeSurfaceNative(surfacePointer: Long, width: Int, height: Int) =
        bridge.cppResizeSurface(surfacePointer, width, height)
    ...
    fun deleteFile(fileHandle: FileHandle) =
        bridge.cppDeleteFile(cppPointer.pointer, nextRequestID.fetchAndIncrement(), fileHandle.handle)
    ...
    fun createDefaultArtboard(fileHandle: FileHandle): ArtboardHandle = ArtboardHandle(
        bridge.cppCreateDefaultArtboard(
            cppPointer.pointer,
            nextRequestID.fetchAndIncrement(),
            fileHandle.handle
        )
    )
    ...
    fun deleteArtboard(artboardHandle: ArtboardHandle) = bridge.cppDeleteArtboard(
        cppPointer.pointer, nextRequestID.fetchAndIncrement(), artboardHandle.handle
    )
     ...
     */
    private val pendingContinuations = ConcurrentMap<Long, CancellableContinuation<Any>>()
    ...
    private suspend inline fun <reified T> suspendNativeRequest(
        ...
        suspendCancellableCoroutine { cont ->
            ...
            pendingContinuations.put(requestID, cont as CancellableContinuation<Any>)
