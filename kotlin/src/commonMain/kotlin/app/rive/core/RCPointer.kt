...
class RCPointer(
    ...
    private var referenceCount: AtomicInt = AtomicInt(1)
    override val refCount: Int
        get() = referenceCount.load()
    ...
    val pointer: Long
        get() {
            check(referenceCount.load() > 0) { "Attempting to access a disposed RCPointer ($label)" }
            return cppPointer
        }
    ...
    override fun release(source: String, reason: String) {
        ...
        RiveLog.v(TAG) {
            "Releasing $label (source: $source$reasonLog; " +
                    "ref count before release: ${referenceCount.load()})"
        }
