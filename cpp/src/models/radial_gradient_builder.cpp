...
void JNIRadialGradientBuilder::apply(jobject paint)
{
	...
	float radius =
	    rive::Vec2D::distance(rive::Vec2D(sx, sy), rive::Vec2D(ex, ey));
	...
	jobject shaderObject = env->NewObject(radialGradientClass,
	...
	jobject newShaderObject =
	    env->CallObjectMethod(paint, getShaderMethodId(), shaderObject);
